...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
         ...
         */
        while (true) {
            val current = account(index)
            if (current is AcquiredAccount && current.op === op) {
                return current
            }

            if (op.completed) return null
            if (current.invokeOperation()) {
                continue
            }

            val acquiredAccount = AcquiredAccount(current.amount, op)
            if (accounts[index].compareAndSet(current, acquiredAccount)) {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val (acq1, acq2) = listOf(
                acquire(min(fromIndex, toIndex), this),
                acquire(max(fromIndex, toIndex), this),
            )
            val from = if (fromIndex < toIndex) acq1 else acq2
            val to = if (toIndex < fromIndex) acq1 else acq2
